...
public class ProfessionManager extends JavaPlugin {
		...
		if (commandSent.equalsIgnoreCase("bpfarmer")) {
			...
			if (currentPlayers != null) {
				for (Player currentPlayer : currentPlayers) {
					if (currentPlayer != null) {
						Profession currentProfession = CurrentPlayerProfessions.get(currentPlayer);

						if (currentProfession != null) {
							player.sendMessage(currentPlayer.getName() + " is a " + currentProfession.professionName);
						}
					}
				}
	...
	public void SetProfession(Player playerToChange, String professionString) {
		...
		playerToChange.sendMessage(PlayerName + " is Profession " + professionString);

		this.saveConfig();
		CurrentPlayerProfessions.put(playerToChange, StringToProfession(professionString, playerToChange));
	}
